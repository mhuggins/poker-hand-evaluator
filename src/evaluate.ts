...
const getOfAKinds = (duplicates: Map<Rank, Card[]>, count: number): Card[][] => {
...
const getAllHandCombinations = ({
  ...
  if (minimumHoleCards <= 0) {
    return getCombinations(holeCards, maximumHoleCards).map((cards) => [...cards, ...communityCards]);
  }
